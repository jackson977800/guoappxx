...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前版本：**0.2.4+10**。
...
全站源版另含“黄果旧版”（cloudfront），共五个独立入口。0.2.4 已补齐这五个入口的独立更新和健康检测，其他功能差异按下方 TODO 继续完善。
...
从首页“更多 → 站源管理”或设置进入独立管理页。每个入口显示缓存数量、分页位置、最近更新时间、任务阶段和错误，可分别更新、继续一页、补齐资料、停止任务和检测；检测结果持久保存。更新合并新增内容并保留历史分页和其他站源缓存，每轮补齐最多 8 部资料，失败或停止保留已完成内容，重复点击不启动重复任务。Android 使用现有前台服务维持后台任务；进程终止后显示中断状态，可再次继续。站源任务只面向编译版本和当前用户获准的入口。
...
| P3-1 | [ ] 应用网络与资源设置 | 应用内直连 / 自动 / 手动代理，目录请求并发与间隔设置。当前使用原生默认网络参数和环境代理，没有完整设置面板；站源健康检测与错误诊断已实现 |
...
0.2.4 已通过 Dart 格式与静态检查、默认 / 全站源两种配置的 Go 竞态检查和 18 项 Python 检查，覆盖独立任务、缓存保留、权限隔离、旧 API 会话、浏览器媒体请求、预览令牌及下载。默认版 69 项 Flutter 检查通过；全站源版 68 项通过，旧 VIP 用例在第五个站源加入后无法定位滚动区外的红果按钮，已修正测试滚动定位，按用户要求未再次运行回归。

用用户截图中的“母女日常”执行原生链路抽样，目录、分集、播放列表和 16 字节密钥均返回 HTTP 200，媒体 Range 请求返回 206。证据在 `build/sources-024/live-huangguo.log`；只读取文本、密钥与媒体开头，未请求海报。这证明开发机当前网络上的原生链路可用，不等同于 Android 实机播放或所有源站网络均已验证。

本轮已生成全站源 ARM64 Release 安装包：`dist/android/zhenguojian-0.2.4+10-arm64-v8a.apk`。已核对版本号、签名和内置原生库；签名与 0.2.3 一致，可覆盖升级。其余版本 / 架构的构建开关和脚本保留，本轮不重复构建。最终构建日志在 `build/sources-024/gradle-finish.log`，包校验记录在 `build/sources-024/apk-verification.log`。
