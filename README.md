...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.10+16** 的源码实施状态。已有解析辅助代码不代表 App 已接入功能；未完成集中验证与平台验收的项目保持未勾选，标明实际实施与检查范围，未实施项目继续按顺序推进。同时记录 App 自身的数据与恢复边界缺陷。优先处理播放可用性、已有数据及用户访问保护，再按原有功能顺序完善。Android 手机和 Windows 同为首要平台，设备验收单独记录。
...
黄果 AI 的实际密钥连接与播放仍未恢复验收。下一批从 P2-3 红果弹幕继续，之后按顺序实施下一集预取、下载合集与剩余媒体流程；已经实现但尚未集中验收的项目不重复列为未实施。
