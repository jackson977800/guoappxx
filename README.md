...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前版本：**0.2.7+13**。
...
全站源版首页按“红果 / 黄豆 / 黄果”显示三个站源。黄果不再显示入口切换，统一汇总视频、AI 与旧 API 的分类和内容；五个实际接口保留各自的权限、分页、独立更新和健康检测，可在站源管理中分别维护。
...
| 功能 | 操作 |
| --- | --- |
| 浏览 | 标题栏切换站源、进入榜单、展开搜索；下方横向滑动分类，内容区左右滑动切类；黄果统一汇总内容，顶部刷新或下拉更新，底部加载更多 |
...
海报在设备内处理和缓存，首页、详情、榜单和追剧共用；有效期 30 天，上限 256 MB / 2000 张，单张上限 20 MB，超限清理较久未使用的图片。过期更新失败时继续显示缓存，失败海报可手动重试。保留 AES / XOR 解码，HEIC 主图片在本地提取后由随包媒体库转成 JPEG，兼容缓存单独限制为 64 MB / 128 张，转换串行执行并清理临时文件。缺失或失效地址会从详情补齐，重复请求合并并设置重试间隔；新图床无需登记，HTTPS 连接失败可尝试可信 DNS 返回的备用地址。
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.7+13** 的修复状态。已有解析辅助代码不代表 App 已接入功能；以下未勾选项仍为待办。同时记录 App 自身的数据与恢复边界缺陷。优先处理播放可用性、已有数据及用户访问保护，再按原有功能顺序完善。Android 手机和 Windows 同为首要平台，设备验收单独记录。
...
从首页“更多 → 站源管理”或设置进入独立管理页。每个入口显示缓存数量、分页位置、最近更新时间、任务阶段和错误，可分别更新、继续一页、补齐资料、停止任务和检测；检测结果持久保存。更新合并新增内容并保留目录页码和其他站源缓存，每轮补齐最多 8 部资料；已完成的条目留在缓存，重复点击不启动重复任务。0.2.7 已修复补齐后的分类副本与菜单持久化；红果重启游标和大目录截断仍待修复，见 P1-R1～P1-R2。Android 使用现有前台服务维持后台任务；进程终止后显示中断状态，可再次继续。站源任务只面向编译版本和当前用户获准的入口。
...
| P1-R4 | [ ] 两版；黄果详情字段另需补齐 | 0.2.7 已加入详情资料回写已有原生目录、封面地址补齐与 HEIC 转换；首页及同 URL 的失败海报随详情 / 播放动作主动重试、黄果完整详情字段仍待进一步贯通。[详情](native/core/app_runtime.go#L497)、[本地记录](lib/local_store.dart#L207)、[海报组件](lib/widgets.dart#L193) | 按 ID 合并并通知首页、分类、详情、收藏与历史，补齐黄果详情字段；显式打开详情 / 播放可触发失败海报重试，附加请求低优先级且不阻塞播放，普通读取不扫描历史资料 |
...
近期 Go 修复同步情况：9 月 18 日红果搜索补漏季尚未完整迁移；9 月 20 日黄果 `Salted__` / 非整分组 AES 和旧 XOR 兼容在 App 已有实现，不能列为整项缺失；详情 / 播放触发失败海报重试尚未贯通；9 月 21 日黄豆明文 JSON 优先识别仍待同步；备用入口海报来源头已在 0.2.7 同步。App 通过 FFI 返回设备文件，没有旧 Go 的 `/api/ui/image` 地址登记网关，应按 P1-R4 处理资料回写，不能照搬服务端 403 路由修复。
...
0.2.7 已生成两份 ARM64 Release 安装包，可覆盖升级。每份的 17 个原生库均使用 ZIP DEFLATE，清单启用安装时解压；签名与 0.2.6 一致，包 CRC、ZIP 对齐、应用名称、版本和内置核心对比均通过，记录在 `build/sources-027/apk-verification.log` 与 `apk-default-verification.log`：

| 版本 | 安装包 | 大小 |
| --- | --- | --- |
| 红果鉴，仅红果 | `dist/android/hongguojian-0.2.7+13-arm64-v8a.apk` | 30.2 MB |
| 真果鉴，全部站源 | `dist/android/zhenguojian-0.2.7+13-arm64-v8a.apk` | 30.2 MB |
