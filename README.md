...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前源码版本：**0.2.12+18（开发快照，待集中验证与平台验收）**；现有红果鉴 / 真果鉴两份 Android ARM64 Release 安装包仍为 **0.2.10+16**。

按用户 2026-09-21 的要求，当前优先完成功能，暂停测试与回归。0.2.12 调整标题栏、VIP 标记、详情及播放器选集布局，并接入预加载、下载合集与偏好、合并队列、Emby 特别篇和网络设置；本轮仅做源码格式、Dart 静态与 Go 核心编译检查，未打包或进行设备验证。0.2.11 的红果弹幕及本轮新增功能尚未包含在现有 APK 中。
...
| 功能 | 操作 |
| --- | --- |
| 浏览 | 标题栏切换站源、排序与筛选、多选下载和展开搜索；窄屏榜单收在“更多 → 榜单”，宽屏保留独立图标。横向滑动分类，内容区左右滑动切类；支持默认、名称、自然季号、上线日期、热度、播放量排序及连载状态筛选。排序作用于已加载内容，未知值置后；“更新剧库”或下拉执行查新、历史续载与资料补齐，底部加载更多 |
...
| 卡片与批量下载 | 发现、追剧和历史卡片的“更多”提供收藏、状态和下载选集；电脑也可右键，电视可用菜单键。发现页长按卡片或点标题栏“多选下载”，标题栏可全选当前内容，选好短剧后进入预览，再按共同画质加入下载 |
...
| 更多 | 红果推荐、站源管理、用户管理、设置与备份、界面模式、关于；设置中提供下载偏好及管理员“网络与资源” |
...
| 黄果 AI（全站源版） | 已接入列表、分类、详情及分集；本轮目录和入口连接超时，历史密钥连接也未通过，播放尚未恢复验收 | 筛选已加载短剧 |
...
解析时限 15 秒，预取最多 6 秒、4 MiB、8 次请求；HLS 限制列表深度和前两段、约 8 秒媒体，MP4 只取开头并在有效范围及版本校验通过后复用。就绪计划保留 2 分钟，失败至少间隔 30 秒再试，避免无限预取或反复解析。

播放器优先从 mpv 实际可跳转缓存区间显示连续缓冲时长与范围；无法取得区间时只显示播放器报告的“已缓冲至”，不把单一终点当作连续范围。当前仅完成静态与核心编译检查，预取复用、网络切换和各平台缓存反馈待集中验证。
...
自动模式读取 Windows 当前用户及 Android / iOS 系统静态代理与排除列表，未配置时使用环境代理或直连；恢复前台及每 30 秒更新。仅配置 PAC 自动脚本而没有可用静态代理时显示配置指引，本版不执行 PAC。代理不会关闭 TLS 校验。Android 与 iOS 的平台桥接已接入源码，尚未完成平台构建与实际网络验证。

目录请求并发可设 1–6、间隔可设 0–5000 毫秒，默认 3 与 250 毫秒；下载并发默认 2、可设 1–6。设置写盘成功后生效，仍保留前台优先、后台让行、取消、退避及请求数量边界。

### 局域网互联：追剧同步与推送播放（待实施）

2026-09-21 新增方案：两台设备在同一局域网直接连接，自动同步追剧 / 播放记录或接续播放，不依赖账号服务器。以下为设计与实施范围，当前 0.2.12 尚未接入此功能。

**发现与自动连接**

App 在前台、当前用户已解锁并允许接收时发布服务，页面提供接收开关。使用独立的 `_zgj-link._tcp` DNS-SD / mDNS 服务类型，发现信息仅包含设备名称、稳定设备 ID、端口、协议版本、证书指纹和同步 / 播放能力；按设备 ID 合并多个网卡地址，不把一台设备显示成多台。
...
首次配对按用户要求自动完成并记录设备证书，后续连接固定校验该指纹；证书改变不静默替换已记住的设备。记录读写使用绑定当前用户与连接的会话令牌，锁定、切换用户或退出接收状态后失效。默认开启当前连接的自动同步，连接成功后启动合并；覆盖本机 / 对方及播放交接由对应按钮触发，不因自动重连而再次执行。

**入口与同步页面**

追剧页标题栏增加带提示文字的“同步”图标，“设置与备份”提供“设备互联”管理入口；发现页现有站源、排序、下载和搜索操作维持当前布局。同步页顶部显示已连接设备和双方当前用户，下方为“自动同步”开关、最近同步状态、待同步 / 冲突数量。连接状态与同步状态分别呈现，可手动“立即同步”；覆盖操作收进“手动同步”次级入口，播放器只保留设备与推送操作。

手动同步面板的三种方式用纵向单选卡片排列，默认选中“双向合并”，底部只放一个主要按钮“开始同步”。每个选项保留一句说明，设备名代入实际名称，操作方向同时用文字和箭头表达。
...
选择后先计算预览，显示两端各自新增、更新、移除、冲突及跳过的数量。覆盖确认明确写出受影响的设备和用户；空列表覆盖也明确显示将清空的数量。同步中保持目标与进度可见，禁用重复提交并提供取消。手机采用适配安全区的页面 / 底部面板；电脑及电视使用居中、限制最大宽度的面板。触控目标至少 48×48，电视增加焦点边框和初始焦点，长设备名换行或省略，系统大字下选项和按钮可增高。
...
没有冲突的记录可自动合并；无法判断先后的并发修改保留两端值并显示“有记录待处理”，其他记录继续同步，播放中不强制弹出冲突窗口。用户可从同步页逐项选择后继续传播结果。自动接收进度只更新记录，不跳转、切集或启动播放器；接续播放仍由“推送”明确触发。自动同步开启时，手动覆盖先暂停当前同步批次，覆盖完成后按新版本继续，避免两种操作交叉写入。

**记录范围与合并规则**

默认范围暂定为追剧列表、想看 / 在看 / 已看及手动已看状态、新集已读基准，以及对应剧目的续播集数和时间。未追剧的全部观看历史不默认加入同步；推送播放可独立接续当前剧，不要求先加入追剧。同步仅针对两端当前已解锁的用户与双方编译版本 / 用户权限共同允许的站源，范围外记录保留并计入跳过数量，不复制密码、账号权限、设备设置、下载索引、视频文件或密钥。

现有 `FollowState` 没有同步修订信息，取消追剧会直接移除条目，因此不能直接交换整个配置备份。新增独立同步格式，以站源和原始剧集 ID 定位记录；追剧成员关系、观看状态与续播进度分别记录修改版本，取消追剧及清空进度保留删除标记。每台设备、每个本地用户使用独立副本标识和递增计数；版本关系用来判断先后，不仅依赖可能不准的设备时间。
...
播放器在常规和全屏控制区提供统一的投放图标，提示文字为“推送”；已连接时显示“推送到［设备名］”，旁边的设备入口用于切换目标。窄屏保留推送和主要播放控制，次要选项放进现有播放设置；推送过程中在控制区显示“正在连接 / 对方准备中 / 已在对方播放”，不在视频中央长期堆叠按钮。

推送内容为站源、剧集 ID、分集 ID / 集数、播放位置与播放意图。对方在自己的权限下读取详情并解析播放地址，优先使用自己的完整本地分集；不发送手机的临时媒体地址、播放密钥或本地路径，也不要求手机持续转发视频。对方缺少该站源或无法播放时返回具体原因，手机保持原播放。仅存在于本机的合并成品暂不提供推送，需要媒体文件传输时另定范围。

交接分为准备与开始两步：对方准备完成后，手机补发最新位置；对方确认已定位并开始播放后，手机才保存进度并暂停。失败、超时或取消时手机继续播放；重复请求使用同一交接 ID，不重复打开播放器。迟到回执不能暂停手机上后来切换的剧集。对方已有其他播放时，在发起端说明将切换的内容。连接与接续播放复用同一设备会话，连接过程无需每次输入配对码。

**技术落点与实施顺序**

优先采用 Flutter `nsd` 进行原生服务发现 / 发布；Go 核心提供独立的局域网 HTTPS 通道、设备身份与有界请求处理，保持现有仅绑定回环地址的媒体服务独立。Flutter 负责配对状态、用户范围、预览与合并，原生请求通过既有 FFI 转交同步事件，实际写入仍由 `LocalStore` 完成。`PlayerScreen` 接入可确认准备 / 开播状态的交接入口，避免只发出导航请求就暂停手机。

实施顺序为同步数据格式与版本 / 删除标记 → 服务发现与自动连接 → 三种同步方式及界面 → 自动增量同步与重连补齐 → 播放推送与交接。Android 手机和 Windows 同步实施，TV 复用协议并适配焦点，iOS 补 Bonjour / 局域网权限并遵守前台接收限制。发现失败保留手动连接，不进行整个网段的地址扫描；平台网络权限和 Windows 入站规则的说明放在连接失败指引中。

参考开源实现与文档（本轮仅查阅文字源码 / 文档）：

- [LocalSend 与协议](https://github.com/localsend/protocol)：参考设备发现、HTTPS 直连、会话和取消流程；它默认使用自定义 UDP 组播，本 App 计划使用原生 DNS-SD / mDNS，不直接声称兼容 LocalSend 协议。
...
0.2.10+16 已完成 Android ARM64 打包与安装包检查，沿用与 0.2.8 相同的本机签名。0.2.12 只检查 Dart 静态与 Go 核心编译，未重新打包；Windows / iOS 完整应用仍待相应平台构建，本轮没有进行设备验收。
...
| Android TV | 与手机共用 APK，旧版电视界面与遥控有自动化记录；0.2.12 选集和设置变更未验证，待电视实机验收 |
| iOS 15.1+ | 已加入工程、Go 核心链接、媒体依赖、文件管理、系统代理桥接和构建脚本；待 Xcode 构建与真机验收，没有已签名 IPA |
...
尚未完成集中验证和平台验收时，使用带 `unverified` 的开发快照 tag。本轮界面与剩余功能源码快照为 `v0.2.12-unverified`，表示源码可恢复，不标为完成验收的正式版本。红果弹幕快照 `v0.2.11-unverified`、0.2.10 功能快照 `v0.2.10-unverified` 与打包修复 `v0.2.10-unverified-task-20260921-055042` 保留。源码同步一致性确认仍执行。

局域网自动连接、记录自动同步与推送播放的方案文档快照为 `v0.2.12-unverified-lan-design`，只增加设计和 P3-2 / P3-3 待办，未增加功能实现，源码版本号保持 0.2.12+18。
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.12+18** 的源码实施状态。已有解析辅助代码不代表 App 已接入功能；未完成集中验证与平台验收的项目保持未勾选，标明实际实施与检查范围。同时记录 App 自身的数据与恢复边界缺陷。Android 手机和 Windows 同为首要平台，设备验收单独记录。

共 29 项：3 项沿用历史已完成标记，23 项已实现但待集中验证，1 项为黄果 AI 播放恢复的外部连接阻塞，2 项为本轮新增的局域网互联方案、待实施。0.2.12 已补入原清单最后六项可继续实施的功能（P2-4、P2-5、P2-6、P2-7、P2-9、P3-1）及标题栏、VIP、详情与播放器源码调整；新增 P3-2 / P3-3 单列，不计入已接入的功能。
...
| P1-5 | [x] 0.2.7 榜单 | 红果总热播 / 真人剧 / 漫剧 / AI 剧四榜；全站源版另有黄豆总榜 / 魔改 / 搜索 / 收藏 / 完结榜和黄果热播 / 推荐 / 潜力榜。保留源站名次与顺序，支持分页、刷新、详情播放；缓存持久化，5 分钟内复用，请求失败可回退 24 小时内的旧榜单，明确提示未更新。编译版本及当前用户权限同时生效；0.2.12 窄屏入口收在标题栏“更多 → 榜单”，宽屏仍有独立图标 |
...
| P3-2 | [ ] 方案已整理，待实施：局域网追剧与播放记录同步 | DNS-SD / mDNS 发现；单设备自动配对连接，多设备列表选择。当前连接默认自动增量合并、断线后补齐，手动提供双向合并 / 覆盖对方 / 覆盖本机；增加版本信息、删除标记、冲突处理、原子保存及失败恢复。Android / Windows 优先，TV 适配焦点，iOS 处理局域网权限 |
| P3-3 | [ ] 方案已整理，待实施：局域网推送播放 | 复用设备连接，播放器“推送”将当前剧集、分集及进度交给对方独立解析播放；对方开播成功后手机暂停，失败继续本机播放。处理单设备自动连接、多设备选择、重复请求、迟到回执和手机 / 全屏 / TV 按钮布局 |

旧项目的 Docker / HTTP 服务、浏览器注册与远程账号、跨设备服务端记录、在线 STRM 签名网关和远程 Emby 定时同步依赖常驻服务器，不直接照搬为本 App 的缺失功能。当前对应能力是设备内用户与备份、原生播放器、本地媒体和 Emby 文件导出；本轮另将不依赖服务器的局域网设备直连、追剧同步与播放接续纳入 P3-2 / P3-3，尚未实施。普通评论在旧项目中也未接入，不列为遗漏的已完成功能。
...
0.2.12+18 已完成本轮界面调整及 P2-4 / P2-5 / P2-6 / P2-7 / P2-9 / P3-1 的源码接入。最终检查结果：
...
本轮没有执行自动化测试、回归、真实媒体播放或设备 / 模拟器验证，也没有构建新 APK、Windows 完整包或 iOS 应用。上述 Go 检查只覆盖核心源码编译，不代表平台桥接、随包媒体库、界面或合并输出已验收。黄果 AI 仅做入口与目录的只读连接诊断，结果超时；未请求站源图片。23 项已实现待验证的状态保持未勾选。

以下保留 0.2.11 及更早版本的检查和安装包记录，不能作为 0.2.12 新增功能的验证结论。
...
Flutter 记录在 `build/danmaku-0211/flutter-default.log` 与 `flutter-all-sources.log`。0.2.11 未进行回归、真实站源弹幕请求、设备或模拟器验证，也未请求站源图片；Android、Windows 与 TV 的实际播放效果待平台验收。0.2.11 尚未打包。
...
黄果 AI 的实际密钥连接与播放仍未恢复验收。0.2.12 时原清单中可独立实施的功能已接入源码；本轮新增的 P3-2 / P3-3 已整理设计，接下来按局域网互联方案实施。AI 待可通的入口后继续处理连接与播放问题，集中验证继续按用户安排执行。
