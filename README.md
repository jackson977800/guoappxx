...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前源码与红果鉴 / 真果鉴两份 Android ARM64 Release 安装包版本：**0.2.14+20（开发快照，待集中验证与平台验收）**。

按用户 2026-09-21 的要求，暂停测试与回归。本轮按打包要求修复初始化后仍停留“正在打开”页面的导航问题，并完成两版 Android ARM64 构建与安装包检查；未运行静态分析、自动化测试、设备或真实局域网验证。新包包含 0.2.11 的弹幕、0.2.12 的界面与媒体流程，以及 0.2.13 的局域网发现、无对端确认的自动连接、追剧与对应续播进度同步和播放推送。
...
0.2.14+20 已完成两版 Android ARM64 打包与安装包检查，签名与 0.2.10+16、0.2.13+19 一致，ARM64 分包版本码递增至 2020，可直接覆盖升级。此版本修复启动页无法切换首页的导航结构，并补齐局域网页面编译所需的导入。Windows / iOS 完整应用仍待相应平台构建，本轮没有进行设备运行确认。

| 平台 | 包与状态 |
| --- | --- |
| Android 8.0+ 手机 | 0.2.14+20 提供两版 `arm64-v8a` APK；ARMv7 / x86_64 构建脚本保留，目前仅有历史版本安装包 |
...
| 当前版本 | 安装包 | 大小 |
| --- | --- | --- |
| 红果鉴，仅红果 | `dist/android/hongguojian-0.2.14+20-arm64-v8a.apk` | 30.9 MB |
| 真果鉴，全部站源 | `dist/android/zhenguojian-0.2.14+20-arm64-v8a.apk` | 30.9 MB |
...
尚未完成集中验证和平台验收时，使用带 `unverified` 的开发快照 tag。本轮启动修复与 Android 打包源码快照为 `v0.2.14-unverified`，表示源码可恢复，不标为完成验收的正式版本。局域网同步与推送快照 `v0.2.13-unverified`、界面与媒体流程快照 `v0.2.12-unverified`、红果弹幕快照 `v0.2.11-unverified`、0.2.10 功能快照 `v0.2.10-unverified` 与打包修复 `v0.2.10-unverified-task-20260921-055042` 保留。源码同步一致性确认仍执行。
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.14+20** 的源码实施状态。已有解析辅助代码不代表 App 已接入功能；未完成集中验证与平台验收的项目保持未勾选，标明实际实施与检查范围。同时记录 App 自身的数据与恢复边界缺陷。Android 手机和 Windows 同为首要平台，设备验收单独记录。
...
打包同时补齐 `lan_screen.dart` 对 `LocalStoreSync` 扩展的导入，解决 Release 编译失败。本轮执行结果：
...
构建和包检查记录在 `build/packages-0214/`，包括两版 `release-*-arm64.log`、`apk-*-verification.log` 和 `apk-verification.json`。本轮没有运行静态分析、自动化测试、回归、设备 / 模拟器或真实局域网验证，没有请求站源图片；包检查不代表启动、同步、推送或其他运行行为已经验收。25 项已实现待验证的状态保持未勾选。
