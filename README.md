...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前版本：**0.2.8+14**。
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.8+14** 的修复状态。已有解析辅助代码不代表 App 已接入功能；以下未勾选项仍为待办。同时记录 App 自身的数据与恢复边界缺陷。优先处理播放可用性、已有数据及用户访问保护，再按原有功能顺序完善。Android 手机和 Windows 同为首要平台，设备验收单独记录。
