...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前源码与红果鉴 / 真果鉴两份 Android ARM64 Release 安装包版本：**0.2.15+21（开发快照，待集中验证与平台验收）**。

按用户 2026-09-21 的要求，暂停测试与回归。0.2.15 调整首页多选、红果推荐和榜单入口，并包含 0.2.14 的启动导航修复。两版 Android ARM64 构建与安装包检查通过；本轮未运行静态分析、自动化测试、设备或真实局域网验证。弹幕、预加载、下载与媒体流程、局域网同步和播放推送均已包含在新包中。
...
| 功能 | 操作 |
| --- | --- |
| 浏览 | 标题栏切换站源、排序与筛选、多选下载和展开搜索；“榜单”固定在分类栏右侧，窄屏也保持可见。横向滑动分类，内容区左右滑动切类；支持默认、名称、自然季号、上线日期、热度、播放量排序及连载状态筛选。排序作用于已加载内容，未知值置后；“更新剧库”或下拉执行查新、历史续载与资料补齐，底部加载更多 |
...
0.2.15 按界面反馈移除首页受 50 部上限限制的全选按钮，多选标题栏改用文字“清空 / 取消”。手机多选时以底部操作栏替换底部导航，选中数量靠左、“下一步”靠右，取消后恢复导航；电脑与电视将操作栏固定在内容区底部。空选时显示点选提示，窄屏大字改为上下排列。红果推荐作为“全部”后的专属分类在当前页切换，“榜单”固定于分类栏右侧，不再收进“更多”。
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.15+21** 的源码实施状态。已有解析辅助代码不代表 App 已接入功能；未完成集中验证与平台验收的项目保持未勾选，标明实际实施与检查范围。同时记录 App 自身的数据与恢复边界缺陷。Android 手机和 Windows 同为首要平台，设备验收单独记录。
...
| P1-5 | [x] 0.2.7 榜单 | 红果总热播 / 真人剧 / 漫剧 / AI 剧四榜；全站源版另有黄豆总榜 / 魔改 / 搜索 / 收藏 / 完结榜和黄果热播 / 推荐 / 潜力榜。保留源站名次与顺序，支持分页、刷新、详情播放；缓存持久化，5 分钟内复用，请求失败可回退 24 小时内的旧榜单，明确提示未更新。编译版本及当前用户权限同时生效；0.2.15 入口固定在分类栏右侧，含文字“榜单”，各尺寸常显，新布局待验证 |
...
0.2.15+21 根据首页截图调整多选与导航入口：移除首页全选，展开后显示文字“清空 / 取消”，底部固定已选数量及“下一步”；仅红果分类在“全部”后提供内嵌推荐，“榜单”固定在分类栏右侧。
...
本轮未执行静态分析、自动化测试、回归、设备 / 模拟器或真实局域网验证，也未请求站源图片；新布局、切换推荐与多选操作的实际运行效果待用户安装确认。25 项已实现待验证的状态保持未勾选。
