...
Flutter 多端独立短剧应用，原名“短剧库 APP”。站源请求、解析、下载和播放均在设备上完成，不依赖旧项目或自建服务。当前源码：**0.2.9+15（功能实现快照，未验证）**。最近已构建安装包仍为 0.2.8+14。

按用户 2026-09-21 的要求，当前优先完成功能，暂停测试与回归，留待以后集中进行。0.2.9 未运行静态检查、自动化、构建或设备验证；源码快照不代表可发布成品。
...
| 功能 | 操作 |
| --- | --- |
| 浏览 | 标题栏切换站源、进入榜单、展开搜索；横向滑动分类，内容区左右滑动切类；排序按钮选择默认、名称、自然季号、上线日期、热度、播放量及连载状态。排序作用于已加载内容，未知值置后；顶部刷新、下拉更新，底部加载更多 |
...
倍速、画质和自动连播开关按本地用户保存，并纳入配置备份；新开播放器沿用偏好。临时 3 倍速不会改写偏好，切集、弹出菜单、失去焦点、转入后台或退出页面时结束临时倍速。关闭自动连播后，播放结束停在当前集；保存失败会提示重试。
...
以下安装包与验收记录属于历史版本。0.2.9 本轮只实施源码，没有生成新 APK、Windows ZIP 或 iOS 包，也没有进行平台验收。
...
按用户最新要求，当前暂停以下检查及回归，待以后集中安排；保留命令供恢复验证时使用，不表示 0.2.9 已执行：
...
暂停验证期间，0.2.9 使用带 `unverified` 的开发快照 tag，只表示源码可恢复，不标为完成验收的正式版本。源码同步一致性确认仍执行。
...
以下对照 `../短剧库` 与红果专用版 `../果果剧库` 的功能和近期修复，基于 2026-09-21 的 0.2.6 审查，并更新至 App **0.2.9+15** 的源码实施状态。已有解析辅助代码不代表 App 已接入功能；本轮以“已实现，待验证”标记代码已接入的项目，暂不勾选验收完成，未实施项目继续按顺序推进。同时记录 App 自身的数据与恢复边界缺陷。优先处理播放可用性、已有数据及用户访问保护，再按原有功能顺序完善。Android 手机和 Windows 同为首要平台，设备验收单独记录。
...
| P2-2 | [ ] 剧库管理与统一更新 | 首页复用站源管理已有的 update 任务，合并为“更新剧库”，兼顾查新、续载和分批补齐历史资料，保持布局紧凑；P1-R1～P1-R4 的源码修复已在 0.2.9 实施、尚待验证。增加卡片收藏 / 下载和多选批量下载。站源管理已能每轮补齐最多 8 部，不能误列为完全没有统一更新；普通读取不主动迁移旧缓存，缺失字段保留未知 |
...
本表保留 0.2.8 及此前状态；0.2.9 新增内容以上方“已实现，待验证”的项目为准。
...
0.2.9+15 本轮实现了播放器全屏菜单、手机横竖屏布局、Go 手势与快捷键、播放偏好，以及分页 / 保存 / 本地用户恢复修复，并继续接入红果推荐、资料模型、排序、全部站源和完整搜索。**按用户要求未运行测试、回归、静态检查、构建或设备验证，也未请求站源图片。** 仅整理源码格式和完成源码镜像一致性确认；未验证的功能不作为已验收版本交付。

黄果 AI 的实际密钥连接与播放仍未恢复验收。后续功能从 P2-1 的追剧 / 历史完善、P2-2 的统一剧库更新继续，弹幕、下一集预取、下载合集与剩余媒体流程等未实施部分继续保留。
